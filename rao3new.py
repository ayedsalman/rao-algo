...
Original file is located at
    https://colab.research.google.com/drive/1nc0A8W4kDc2uZmQZqQ0YG77nx4ihzzjo
"""
    ...
    for i in range(0, SearchAgents_no):
      if (f1[i] < f1[r]): 
        for j in range(0, dim):
            ...
            Positions[i,j]= Positions[i,j] + r1*(best_pos[j]-np.abs(worst_pos[j])) + r2*(np.abs(Positions[i,j])-Positions[i,j])#change in position
            Positions[i,j]=np.clip(Positions[i,j], lb[j], ub[j]) 
        else :
                  for j in range (0,dim):
                        r1=random.random() # r1 is a random number in [0,1]
