...
Original file is located at
    https://colab.research.google.com/drive/1f9cG4NujjvTlfvZ7CwIUyspPHVOkorTE
"""
    ...
    for i in range(0, SearchAgents_no):
      if (f1[i] < f1[r]): 
        for j in range(0, dim):
            ...
            Positions[i,j]= Positions[i,j] + r1*(best_pos[j]-worst_pos[j]) + r2*(np.abs(Positions[i,j])-np.abs(Positions[i,j])) #change in position
            Positions[i,j]=np.clip(Positions[i,j], lb[j], ub[j]) 
